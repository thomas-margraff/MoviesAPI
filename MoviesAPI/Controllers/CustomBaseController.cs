...
namespace MoviesAPI.Controllers
{
    public class CustomBaseController : ControllerBase
    {
        ...
        {
            var entity = await queryable.AsNoTracking().FirstOrDefaultAsync(r => r.Id == id);
            if (entity == null)
            {
                return NotFound();
            }
        ...
        protected async Task<ActionResult> Post<TCreation, TEntity, TRead>([FromBody] TCreation creation, string routeName) 
        ...
        protected async Task<ActionResult> Put<TCreation, TEntity>(int id, [FromBody] TCreation creation)
            where TEntity : class, IId
        {
            var entity = mapper.Map<TEntity>(creation);
            entity.Id = id;
        ...
        {
            var exists = await context.Set<TEntity>().AnyAsync(r => r.Id == id);
            if (!exists)
            {
                return NotFound();
            }
            ...
            where TEntity : class, IId
        {
            ...
            var entityFromDB = await context.Set<TEntity>().FirstOrDefaultAsync(r => r.Id == id);
